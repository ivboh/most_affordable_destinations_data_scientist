...
def find_zipcode_in_major_cities():
    ...
    ('Houston', 'TX'), ('McLean', 'VA')]
    zipsearch = SearchEngine(simple_zipcode=False)
    l=[]
    for i in range(500, 100000):
        temp = zipsearch.by_zipcode(str(i).zfill(5))
        if temp.zipcode and (temp.city,temp.state) in major_cities: l.append([temp.zipcode, temp.city]) 
    return l
...
def savejkbylocation(zipcode):
    ...
    for start in range(0,100000,50):
        URL= 'https://www.indeed.com/jobs?q=("data+science"+or+"data+scientist")&l={}&jt=fulltime&radius=5&filter=0&limit=50&start=0'.format(zipcode)
